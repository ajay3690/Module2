...
double norm(double **A); // norm of a single matrix
double **tr_mul_mat(double **A, double **B, double **C);// transpose matrix *matrix
double Angle(double **A, double **B,double norm1, double norm2); // angle
double square_root(double **a);//square root
//End function declaration

// norm of a single matrix
double norm(double **A){
	double **E1;
	E1=Matmul(transposeMat(A,3,1),A,1,3,1);
	double e=square_root(E1); //e =vn1
	return e;
}

// transpose matrix * matrix
double **tr_mul_mat(double **A, double **B,double **C){
	double **b1,**b111,k;
	b1=Matmul(transposeMat(A,3,1),B,1,3,1);//v2.T @ A
	k= b1[0][0];
        b111=Matscale(C,3,1,k);// (v2.T @A )* A
	return b111;
}

//Angle 
double Angle(double **A,double **B,double norm1,double norm2){
	double **T1,t1,theta1;
	T1=Matscale(Matmul(transposeMat(A,3,1),B,1,3,1),1,1,1/(norm1*norm2));
	t1=T1[0][0];
	theta1=(180/M_PI)*acos(t1);
	return theta1;
}
