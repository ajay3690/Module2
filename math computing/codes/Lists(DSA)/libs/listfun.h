...
avyuh *tr_mul_mat(avyuh *A, avyuh *B,avyuh *C);
...
avyuh *tr_mul_mat(avyuh *A, avyuh *B,avyuh *C){
	avyuh *b1,*b111;
	double k;
	b1=Listmul(transposeList(A),B);//v2.T @ A
	k=b1->vector->data;
        b111=Listscale(C,k);// (v2.T @A )* A
	return b111;
}

double Angle(avyuh *A,avyuh *B,double norm1,double norm2){
	avyuh *T1;
	double t1,theta1;
	T1=Listscale(Listmul(transposeList(A),B),1/(norm1*norm2));
	t1=T1->vector->data;
...
{
	int i;
avyuh *head = createList(3,3);
sadish *r1,*r2,*r3;
r1      =head->vector;
r2      =head->next->vector;
r3	=head->next->next->vector;
...
r3->data             =Vecind(a->vector,2)->data;
r3->next->data       =Vecind(b->vector,2)->data;
r3->next->next->data =Vecind(c->vector,2)->data;
